...
def filtering_category(database: dict,
    ...
    if category_key is not None:
        result = [database[product] for product in database if database[product]['category'] == category_key]  # TODO При помощи фильтрации в list comprehension профильтруйте товары по категории. Или можете использовать
        # result = list(filter(lambda product: product['category'] == category_key, DATABASE.values()))
    else:
        result = [product for product in database.values()]  # TODO Трансформируйте database в список словарей
